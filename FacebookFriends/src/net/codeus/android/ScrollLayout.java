...
public class ScrollLayout extends LinearLayout {
	...
	private TimeTextView mCenterView;
	...
	public void setLabeler(DateSlider.Labeler labeler, long time, int objwidth, int objheight) {
		...
		while (displayWidth>childrenWidth-0*objWidth && labeler!=null) {
			...
			if (childrenWidth==0) {
				TimeTextView ttv = labeler.createView(getContext(),true);
				ttv.setVals(labeler.getElem(currentTime));
				addView(ttv,lp);
				mCenterView = ttv;
				childrenWidth += objWidth;
			}
			TimeTextView ttv = labeler.createView(getContext(),false);
			ttv.setVals(labeler.add(((TimeTextView)getChildAt(getChildCount()-1)).endTime,1));
			addView(ttv,lp);
			ttv = labeler.createView(getContext(),false);
			ttv.setVals(labeler.add(((TimeTextView)getChildAt(0)).endTime,-1));
			addView(ttv,0,lp);
	...
	public void setTime(long time, int loops) {
		...
		TimeTextView currelem = (TimeTextView)getChildAt(pos);
		if (loops>2 || currelem.startTime <= time && currelem.endTime >= time) {
			if (loops>2) {
				Log.d(TAG,String.format("time: %d, start: %d, end: %d", time, currelem.startTime, currelem.startTime));
				return;
			}
			double center = getWidth()/2.0;
			int left = (getChildCount()/2)*objWidth-getScrollX();
			double currper = (center-left)/objWidth;
			double goalper = (time-currelem.startTime)/(double)(currelem.endTime-currelem.startTime);
			int shift = (int)Math.round((currper-goalper)*objWidth);
			mScrollX-=shift;
			reScrollTo(mScrollX,0,false);
		}
		else {
			double diff = currelem.endTime - currelem.startTime;
			int steps = (int)Math.round(((time-(currelem.startTime+diff/2))/diff));
			moveElements(-steps);
	...
	protected void reScrollTo(int x, int y, boolean notify) {
		...
		if (listener!=null && notify) {
			...
			long newTime = (long)(mCenterView.startTime+(mCenterView.endTime-mCenterView.startTime)*f);
	...
	protected void moveElements(int steps) {
		if (steps<0) {
			for (int i=0;i<getChildCount()+steps;i++) {
				((TimeTextView)getChildAt(i)).setVals((TimeTextView)getChildAt(i-steps));
			}
			for (int i=getChildCount()+steps;i>0 && i<getChildCount();i++) {
				DateSlider.TimeObject newTo = labeler.add(((TimeTextView)getChildAt(i-1)).endTime,1);
				((TimeTextView)getChildAt(i)).setVals(newTo);
			}
			if (getChildCount() + steps <= 0) {
				for (int i=0;i<getChildCount();i++) {
					DateSlider.TimeObject newTo = labeler.add(((TimeTextView)getChildAt(i)).endTime,-steps);
					((TimeTextView)getChildAt(i)).setVals(newTo);
				}
			}
		} else if (steps > 0) {
